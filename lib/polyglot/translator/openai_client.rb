...
#{text}"
        ...
        def openai_chat(prompt, system: nil)
          ...
          conn = Faraday.new(url: 'https://api.openai.com', request: { timeout: Polyglot.config.timeout, open_timeout: 5 }) do |f|
            f.request :json
            f.adapter Faraday.default_adapter
          end

          body = {
            model: Polyglot.config.model,
            messages: [
          ...
          if res.status >= 400
            raise Polyglot::Error, "OpenAI API error: #{res.status} - #{res.body}"
          end
