    ...
    class GeminiClient < Base
      class << self
        ...
        def gemini_generate(prompt, system: nil)
          ...
          model = Polyglot.config.model || 'gemini-1.5-flash'

          conn = Faraday.new(url: 'https://generativelanguage.googleapis.com', request: { timeout: Polyglot.config.timeout, open_timeout: 5 }) do |f|
            f.request :json
          ...
          if res.status >= 400
            raise Polyglot::Error, "Gemini API error: #{res.status} - #{res.body}"
          end
